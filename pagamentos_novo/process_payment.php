...
if ($_GET["acc"] == "check") {
    ...
    //atualizar a tabela vendas com o id do mercado pago
    $pdo->query("UPDATE matriculas SET ref_api = '$id_mercadopago' where id = '$id_venda' ");

    // pix
    if($payment_method_id=="pix"){ 
        $forma_pgto = 'Pix';     
        $pdo->query("UPDATE matriculas SET forma_pgto = 'Pix' where id = '$id_venda' ");

    }else{
         $forma_pgto = 'MP';    
         $pdo->query("UPDATE matriculas SET forma_pgto = 'MP' where id = '$id_venda' ");
    }
    ...
    if ($status == "approved") { // PAGAMENTO APROVADO
        ...
        //atualizar a tabela vendas para pago sim
        $id_matricula = $id_venda;

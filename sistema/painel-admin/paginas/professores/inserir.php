...
if ($id == "") {
    ...
    if ($comissao === null) {
        $query = $pdo->prepare("SELECT porcentagem FROM comissoes WHERE nivel = 'Professor' LIMIT 1");
        $query->execute();
    ...
    $query = $pdo->prepare("INSERT INTO usuarios SET  wallet_id = :wallet_id, nome = :nome, usuario = :email, senha = '$senha', cpf = :cpf, senha_crip = '$senha_crip', nivel = 'Professor',  foto = '$foto', id_pessoa = '$ult_id', ativo = 'Sim', data = curDate()");
    ...
    $query = $pdo->prepare("UPDATE usuarios SET wallet_id = :wallet_id, nome = :nome, usuario = :email, cpf = :cpf, foto = '$foto' WHERE id_pessoa = '$id' and nivel = 'Professor'");

...
class Stylesheet
{
    ...
    private function _css_selector_to_xpath($selector, $first_pass = false)
    {
        ...
        // Initial query (non-absolute) ?? $query = "//";
        ...
        while ($i < $len) {
            ...
            switch ($s) {
                ...
                case "[":
                    // Attribute selectors. ?? All with an attribute matching the following token(s) ?? $attr_delimiters = ["=", "]", "~", "|", "$", "^", "*"];
                    ...
                    switch ($op) {
                        ...
                        case "~=":
                            ...
                            // (e.g. [type~="a b c" "d e f"]) ?? $values = explode(" ", $value);
                            $query .= "[";
...
//    }
    ...
    function apply_styles(FrameTree $tree)
    {
        ...
        // iterate over the tree using an implicit FrameTree iterator.) ?? $root_flg = false;
    ...
    private function _parse_properties($str)
    {
        ...
        foreach ($properties as $prop) {
            ...
            /*if (strpos($prop, "url(") === false) {
              if (preg_match("/([a-z-]+)\s*:\s*[^:]+$/i", $prop, $m)) ?? $prop = $m[0];
            }*/

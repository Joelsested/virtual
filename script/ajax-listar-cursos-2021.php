...
require_once("busca-utils.php");

$termos = preparar_padrao_busca($_POST['busca'] ?? '');
...
$camposBusca = ['nome', 'desc_rapida'];
$clausulaBusca = clausula_busca_sem_acento($camposBusca, $termos['original'], $termos['sem_acento']);

$query = $pdo->query("SELECT * FROM cursos where status = 'Aprovado' and sistema = 'N?o' and ano = '2021' and $clausulaBusca ORDER BY id desc LIMIT $limite, $itens_pag");
...
for($i=0; $i < $total_reg; $i++){
    ...
    $query2 = $pdo->query("SELECT * FROM cursos where status = 'Aprovado' and sistema = 'N?o' and ano = '2021' and $clausulaBusca ORDER BY id desc ");
    ...
    $query2 = $pdo->query("SELECT * FROM cursos where status = 'Aprovado' and sistema = 'Não' and ano = '2021' and $clausulaBusca ORDER BY id desc ");
    $res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
    $total_reg2 = @count($res2);
    ...
    <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3 portfolio-item">
                    <div class="portfolio-one">
                        <div class="portfolio-head">
                            <div class="portfolio-img2"><img alt="" src="sistema/painel-admin/img/cursos/{$foto}"></div>
                            <div class="portfolio-hover">
                               <iframe class="video-card" src="{$primeira_aula}" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                <div class="" align="center" style="margin-top:20px; ">                   
                                     <a href="curso-de-{$url}" type="button" class="btn btn-primary2">Veja Mais <i class="fa fa-caret-right"></i></a>
                                 </div>

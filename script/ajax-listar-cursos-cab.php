...
$query = $pdo->query("SELECT * FROM pacotes where $clausulaBusca ORDER BY nome asc");
...
$query = $pdo->query("SELECT * FROM cursos where status = 'Aprovado' and sistema = 'Não' and $clausulaBusca ORDER BY nome asc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_reg = @count($res);
if($total_reg > 0){

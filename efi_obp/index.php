...
if ($id_pessoa) {
    $consulta_usuario_aluno = $pdo->query("SELECT usuario FROM alunos where id = '$id_pessoa' ");
    $reposta_cosulta_usuario_aluno = $consulta_usuario_aluno->fetchColumn();

    $consulta_nivel_responsavel = $pdo->query("SELECT * FROM usuarios where id = '$reposta_cosulta_usuario_aluno' ");
...
if ($forma_de_pagamento == 'BOLETO_PARCELADO' and $quantidadeParcelas > 1) {
    ...
    if (@count($res) > 0) {
        ...
        $dadosBoleto = [
            ...
            'quantidade' => 1,
            'nome' => $res2[0]['nome'] ?? '',
            'email' => 'sestedcursosvirtual@gmail.com',
            'cpf' => $res2[0]['cpf'] ?? '',
            ...
            'notification_url' => 'https://sested.sestedsaas.com/efi_webhook_boleto_parcelado.php'
        ];
